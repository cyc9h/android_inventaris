...
public class InventarisData {
    ...
    public static ArrayList<Inventaris> getListData(){
        Inventaris inventaris = null;
        ArrayList<Inventaris> list = new ArrayList<>();
        for (int i = 0; i <data.length; i++) {
            inventaris = new Inventaris();
            inventaris.setId(data[i][0]);
            inventaris.setNama(data[i][1]);
            //inventaris.setJumlah(data[i][2]);
            inventaris.setPhoto(data[i][2]);
